...
class WinningStrategy(Tk):
    ...
    def search_tree_help(self):
        tkinter.messagebox.showinfo('Search tree',
            ...
            'Control click on a state to collpase the tree and hide all displayed possible '
            ...
            'The outline of a state is dashed boldfaced if that state is part of a winning strategy '
            'at some point in the game, but not from its beginning; '
    ...
    def display_state(self, canvas, line):
        for i in range(3):
            for j in range(3):
                ...
                y = OFFSET + line * WIDTH
                canvas.create_polygon(x + 1, y + 1, x + WIDTH - 1, y + 1, x + WIDTH - 1, y + WIDTH - 1,
                                      x + 1, y + WIDTH - 1, fill = '',
                                      outline = {0: 'black', 1: 'red'}[self.depth % 2],
                                      width = {ON_TRACK_TO_LOSE: 1, ON_TRACK_FOR_LUCKY_WIN: 2,
                                               ON_TRACK_TO_WIN: 2}[self.expected_outcome],
                                      dash = {ON_TRACK_TO_LOSE: 1, ON_TRACK_FOR_LUCKY_WIN: 3,
                                               ON_TRACK_TO_WIN: 1}[self.expected_outcome],
                                      dashoff = 1)
                canvas.create_oval(x + j * THIRD_WIDTH + 3, y + i * THIRD_WIDTH + 3,
                                   x + j * THIRD_WIDTH + 8, y + i * THIRD_WIDTH + 8,
                                   fill = {BLACK: 'black', RED: 'red', FREE: 'white'}[self.state[i][j]],
                                   outline = 'black')
                self.x = x
                self.y = y
...
class DisplayedSearchTree(Frame):
    ...
    def expand_or_collapse_tree(self, event, expand):
        ...
        while states:
            state = states.pop()
            if (state.displayed and state.x < self.displayed_tree.canvasx(event.x) < state.x + WIDTH
                                   and state.y < self.displayed_tree.canvasy(event.y) < state.y + WIDTH):
                found_state = state
                break
...
class Game:
    ...
    def learn(self, displayed_games = None, game_nb = None):
        return self.play(self.search_tree, RED, displayed_games, game_nb)
    ...
    def well_pruned(self, tree):
        if (tree.expected_outcome != ON_TRACK_TO_WIN or
    ...
    def learn(self, random_player):
        ...
        if learning_over:
            self.displayed_wins.create_text(125, 445,
                                   text = 'Learning was completed\nafter {} games'.format(learning_over))
        else:
            for i in range(100, 1000):
                self.game.learn()
                if self.game.well_pruned(self.game.search_tree):
                    self.displayed_wins.create_text(125, 445,
                                                  text = 'No more to learn after {} games'.format(i + 1))
                    learning_over = True
                    break
