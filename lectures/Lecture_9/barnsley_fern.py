...
class BarnsleyFern(Tk):
    def __init__(self):
        ...
        Button(draw_buttons, text = 'Draw custom fern',
               command = lambda fern_species = 'Custom' :
                                            self.display_fern(fern_species)).pack(padx = 20, side = LEFT)
        draw_buttons.pack()
        self.displayed_fern = StringVar()
        Label(draw_buttons, textvariable = self.displayed_fern, width = 16,
              fg = FERN_COLOUR).pack(pady = 40)
    ...
    def affine_transformations_help(self):
        tkinter.messagebox.showinfo('Affine transformations',
            ...
            'each of wich maps a point (x, y) to (ax + by + e, cx + dy + f) for some real numbers '
            'a, b, c, d, e and f, i.e., A is the union of T1(A), T2(A), T3(A) and T4(A).\n\n'
...
class Drawing(Frame):
    ...
    def display_fern(self, fern_species):
        if fern_species == 'Custom':
            ...
            self.fern = {'Black Spleenwort': BlackSpleenwort(), 'Culcita': Culcita(),
                         'Cyclosorus': Cyclosorus()}[fern_species]
    ...
    def transform(self, linear_transformation_matrix, translation_vector, x, y):
        new_x = (linear_transformation_matrix[0][0] * x +
                 linear_transformation_matrix[0][1] * y + translation_vector[0])
        new_y = (linear_transformation_matrix[1][0] * x +
                 linear_transformation_matrix[1][1] * y + translation_vector[1])
        self.draw_point(new_x, new_y)
        return new_x, new_y
...
class Fern:
    def __init__(self,
                 linear_coefficients = [[[None, None], [None, None]], [[None, None], [None, None]],
                                        [[None, None], [None, None]], [[None, None], [None, None]]],
                 translation_vectors = [[None, None], [None, None], [None, None], [None, None]],
                 probas = [None, None, None, None],
                 factor = None):
...
class FernParametersBoard(Frame):
    def __init__(self):
        ...
        text_labels = {0: 'Stem', 1: 'Upper leaflets', 2: 'Lower left leaflet', 3: 'Lower right leaflet'}
        for n in range(4):
            Label(self, text = text_labels[n]).grid(row = 3 * n, columnspan = 4, pady = 5)
            for i in range(2):
                for j in range(2):
