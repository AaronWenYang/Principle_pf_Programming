...
if not os.path.isfile(original_form_name):
    print('I could not find {}. Giving up...'.format(original_form_name))
    sys.exit()
...
if os.path.isfile(form_name):
    print('There is already a file named {}. Giving up...'.format(form_name))
    sys.exit()
...
try:
    ...
    print('Could not open {}. Giving up...'.format(form_name))
    sys.exit()

print('\nOpen the file {}.'.format(form_name))
print('When you are ready to position the mouse above the Student ID field')
input('within {} seconds (not moving the mouse then), press return... '.format(sleeping_time))
time.sleep(sleeping_time)
student_id_field = pyautogui.position()
pyautogui.click(student_id_field[0], student_id_field[1])
pyautogui.typewrite(student_id + '\t' +
                    family_name + '\t' + given_names + '\t' +
                    program + '\t' + phone_number + '\t' +
                    academic_standing_level + '\t' + semester_and_year + '\t' * 3 +
                    submission_date + '\t' * 9)
...
input('within {} seconds, press return... '.format(sleeping_time))
time.sleep(sleeping_time)
program_authority_name_field = pyautogui.position()
pyautogui.click(program_authority_name_field[0], program_authority_name_field[1])
