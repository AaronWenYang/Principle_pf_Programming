...
def solve(cryptarithm):
    ...
    all_letters_with_those_starting_a_word_last =\
                    ''.join(letters_not_starting_a_word) +\
                    ''.join(letters_starting_a_word)
    if len(all_letters_with_those_starting_a_word_last) > 10:
        print('There are more than 10 letters, '
              'this is not a valid cryptarithm.')
        return
    for possible_solution in generate_possible_solutions(
                                    [str(i) for i in range(10)],
                                    len(letters_starting_a_word),
                                    len(letters_not_starting_a_word)):
        equation = cryptarithm.translate(str.maketrans(
             all_letters_with_those_starting_a_word_last, possible_solution))
        try:
            if eval(equation): 
                print(equation)
        ...
        if size > 1:
            ...
            if size % 2 or size == 2:
               L[start], L[start + size - 1] = L[start + size - 1], L[start]
                ...
                L[start: start + 2],\
                L[start + 2: start + size - 2],\
                L[start + size - 2],\
                L[start + size - 1] = \
                         L[start + size - 3: start + size - 1],\
                         L[start + 1: start + size - 3],\
                         L[start + size - 1], L[start]
    else:
        size -= 1
        length -= 1
        for i in range(size):
