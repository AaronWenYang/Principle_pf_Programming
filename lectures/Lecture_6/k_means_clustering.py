...
class KMeansClustering(Tk):
    def __init__(self):
        ...
        self.space = Space()
        buttons = Frame(bd = 20)
        self.configure_space_or_cluster_button = Button(buttons, text = 'Cluster', width = 5,
                                                        command = self.configure_space_or_cluster)
        self.configure_space_or_cluster_button.pack(padx = 30, side = LEFT)
        self.clear_or_iterate_button = Button(buttons, text = 'Clear', width = 5,
...
class Space(Frame):
    def __init__(self):
        ...
        self.space = Canvas(self, width = SPACE_DIM, height = SPACE_DIM, bg = SPACE_COLOUR)
    ...
    def clear(self):
        if self.centroids:
            ...
            for point_coordinates in self.points:
                ...
                self.space.itemconfig(self.points[point_coordinates].drawn_point, fill = POINT_COLOUR,
    ...
    def iterate(self):
        clusters = dict((centroid_coordinates, []) for centroid_coordinates in self.centroids)
        if not clusters:
            return
